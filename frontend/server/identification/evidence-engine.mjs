...
- Record visualFeatures such as border color, foil pattern, frame layout, and reflective finish without naming a parallel unless the evidence supports that name. Treat surface finish and repeating foil geometry as separate observations.
- Inspect shiny cards for repeatable holographic or refractor structure: rainbow refraction, prism facets, dots or sparkle, speckle, confetti, cracked-ice or shattered polygons, waves, rays, circles, checkerboard, X patterns, atomic bursts, camouflage, and other repeated geometry. Record pattern direction, scale, coverage, and whether it is card-wide or localized. Describe only what is visibly supported; do not map a generic pattern to a trademarked parallel name from appearance alone.
- Distinguish a real surface pattern from sleeve glare, scanner streaks, shadows, camera reflections, and one isolated highlight. A repeated pattern across separated card regions or consistent geometry is stronger evidence. If the angle hides the finish, mark it uncertain rather than calling the card base.
- Record the pictured subject's pose or action as its own visualFeature. Use a concrete normalized description such as batting swing, batting ready stance, pitching delivery, running, fielding ready, throwing, catching, sliding, posed portrait, or unknown. Include visible orientation or handedness only when clear. Distinguish an active swing from merely holding a bat, and do not infer a field position that is not visibly supported.
- Treat a same-player pose as a discriminating image fingerprint. Candidate suggestions must say whether the pictured action and body orientation agree with the candidate; a different pose is a material conflict even when player, team, and card design are otherwise similar.
...
- Treat the combination of exact serial denominator, border color, surface finish, foil geometry, pattern direction, and printed parallel text as a visual fingerprint. Candidate suggestions must explicitly compare these attributes. A denominator or independently cataloged pattern that agrees with a known candidate is strong corroboration, but color, shine, or pattern alone must never name a parallel.
...
- Treat year as the card's issue year or printed sports season. Preserve a complete season exactly in normalized form (for example, 2024-25 rather than only 2024) when the product title or card text supports both years.
- Inspect the front product line and the back copyright/manufacturer line separately for date evidence. A copyright year is evidence, but it must not replace a clearly printed issue season; if the product season and copyright date differ, report both observations and use the product season for year.
