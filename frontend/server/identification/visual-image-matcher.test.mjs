...
import { VisualImageMatcher, visualImageInternals } from "./visual-image-matcher.mjs";

async function cardImage({ border, panel, stripe }) {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="480" height="680">
    ...
    <circle cx="240" cy="340" r="120" fill="#334455"/>
  </svg>`;
...
test("independent visual matcher separates matching borders and layouts", async () => {
  ...
  const different = await cardImage({ border: "#1769aa", panel: "#d8e6ef", stripe: 500 });
