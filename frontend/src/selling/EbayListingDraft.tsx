...
type SellerSetup = {
  ...
  fulfillmentPolicies: { id: string; name: string; buyerShippingCostCents?: number | null; shippingService?: string }[];
...
export function EbayListingDraft({ card, onClose }: { card: SavedCollectionCard; onClose: () => void }) {
  ...
  const [listingCostSafety, setListingCostSafety] = useState<{ enabled: boolean; transactionFeePercent: number; transactionFixedFeeCents: number; mailingCostCents: number; estimatedBuyerSalesTaxPercent: number } | null>(null);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (current) {
        ...
        setListingCostSafety(draftPayload.listingCostSafety ?? null);
  ...
  const selectedBuyerShippingCents = setup?.fulfillmentPolicies.find((policy) => policy.id === draft?.fulfillmentPolicyId)?.buyerShippingCostCents ?? 0;
  const proceedsSubtotalCents = referencePriceCents + selectedBuyerShippingCents;
  const estimatedBuyerSalesTaxCents = Math.round(proceedsSubtotalCents * ((listingCostSafety?.estimatedBuyerSalesTaxPercent ?? 7) / 100));
  const proceedsFeeBasisCents = proceedsSubtotalCents + estimatedBuyerSalesTaxCents;
  const estimatedFeeCents = Math.round(proceedsFeeBasisCents * ((listingCostSafety?.transactionFeePercent ?? 13.25) / 100)) + (listingCostSafety?.transactionFixedFeeCents ?? 30);
  const promotionFeeCents = draft?.promoteListing
    ? Math.round(proceedsFeeBasisCents * ((draft.promotionAdRatePercent ?? 2) / 100))
    : 0;
  const estimatedShippingLabelCents = listingCostSafety?.mailingCostCents ?? 78;
  const estimatedProceedsCents = proceedsSubtotalCents - estimatedFeeCents - promotionFeeCents - estimatedShippingLabelCents;
  const safetyNetCents = estimatedProceedsCents;
  const belowCostWarning = listingCostSafety?.enabled && safetyNetCents < 0
    ? `Do not sell at a loss is on: this price is estimated to lose $${(Math.abs(safetyNetCents) / 100).toFixed(2)} after the selected shipping, eBay fees, promotion, tax fee effect, and mailing cost.`
    : null;
  ...
  return (
    <div className="ebay-draft-backdrop" role="presentation">
      <section className="ebay-draft-panel" role="dialog" aria-modal="true" aria-labelledby="ebay-draft-title">
        <header><div><span>{status?.environment === "production" ? "Live eBay selling" : "Sandbox-first selling"}</span><h2 id="ebay-draft-title">Sell {card.title} on eBay</h2></div><button className="ebay-secondary-action" type="button" onClick={onClose}>Close</button></header>
        {!draft || !status ? <div className="collection-empty"><span className="spinner" /> Preparing draft...</div> : (
          <>
            <div className={`ebay-connection-status ${status.connected ? "connected" : ""}`}>
              ...
              {status.connected && <button className="ebay-secondary-action" type="button" disabled={busy} onClick={() => void reconnect()}>Reconnect eBay permissions</button>}
            </div>
            ...
            {belowCostWarning && <div className="error-banner ebay-setup-feedback" role="alert">{belowCostWarning}</div>}
            ...
            <section className="ebay-proceeds"><div><span>Price reference</span><strong>{draft.currency} {(referencePriceCents / 100).toFixed(2)}</strong></div>{selectedBuyerShippingCents > 0 && <div><span>Buyer-paid shipping</span><strong>+ {draft.currency} {(selectedBuyerShippingCents / 100).toFixed(2)}</strong></div>}<div><span>Estimated eBay transaction fee</span><strong>− {draft.currency} {(estimatedFeeCents / 100).toFixed(2)}</strong></div>{draft.promoteListing && <div><span>Maximum promotion fee ({draft.promotionAdRatePercent ?? 2}%)</span><strong>− {draft.currency} {(promotionFeeCents / 100).toFixed(2)}</strong></div>}<div><span>Estimated shipping label</span><strong>− {draft.currency} {(estimatedShippingLabelCents / 100).toFixed(2)}</strong></div><div><span>Approximate proceeds</span><strong>{estimatedProceedsCents < 0 ? "− " : ""}{draft.currency} {(Math.abs(estimatedProceedsCents) / 100).toFixed(2)}</strong></div>{listingCostSafety?.enabled && <p className="account-inline-success"><strong>Do not sell at a loss is on.</strong> Before CardPilot publishes or changes the live price, it uses this same estimate: the selected shipping policy, transaction and promotion fees, buyer-tax fee effect, and your ${(estimatedShippingLabelCents / 100).toFixed(2)} shipping-label cost.</p>}<p>Estimate includes buyer-paid shipping, when applicable, and deducts the seller-paid label even when shipping is free to the buyer. Fees use {(listingCostSafety?.transactionFeePercent ?? 13.25).toFixed(2)}% plus ${((listingCostSafety?.transactionFixedFeeCents ?? 30) / 100).toFixed(2)}{draft.promoteListing ? ", plus the selected promotion rate when eBay attributes the sale to the ad" : ""}. Actual fees and label prices may vary by seller, category, promotions, taxes, shipping service, and final auction price.{draft.listingFormat === "AUCTION" ? ` Selected auction currently ends around ${auctionEnd.toLocaleString()}.` : ""}</p></section>
